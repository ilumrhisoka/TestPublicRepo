...
public interface SessionRepository extends JpaRepository<Session, Long> {
    ...
    @Query(value = "SELECT s.* FROM sessions s JOIN users u ON s.user_id = u.id WHERE u.deleted = FALSE AND s.ended_at_utc < :endDate ORDER BY s.started_at_utc DESC", nativeQuery = true)
